DROP DATABASE IF EXISTS classwork4;
...
SELECT 
	id,
	CONCAT(first_name, ' ', last_name) AS 'Пользователь', 
...
WHERE
    user_id = (SELECT 
        ...
        FROM
            users
        WHERE
            email = 'arlo50@example.org')
        AND media_type_id IN (SELECT 
            id
        FROM
            media_types
        WHERE
            name_type LIKE 'photo'); 
...
SELECT 
	u.id,
	CONCAT(u.first_name, ' ', u.last_name) AS 'Пользователь', 
	p.hometown AS 'Город',
...
SELECT 
	m.id,
	m.filename AS 'медиа',
	CONCAT(u.first_name, ' ', u.last_name) AS 'владелец медиа',	
...
FROM friend_requests WHERE status = 'approved' AND target_user_id = 1
UNION
SELECT target_user_id AS id
FROM friend_requests WHERE status = 'approved' AND initiator_user_id = 1 )
SELECT fr.initiator_user_id AS friend_id
FROM friends f JOIN friend_requests fr ON fr.target_user_id = f.id
WHERE fr.initiator_user_id != 1 AND fr.status = 'approved'
UNION
SELECT fr.target_user_id
FROM friends f JOIN friend_requests fr ON fr.initiator_user_id = f.id
...
SELECT 
    fr.initiator_user_id AS friend_id
FROM
    v_friends f
        JOIN
    lesson_4.friend_requests fr ON fr.target_user_id = f.friend_id
WHERE
...
FROM
    v_friends f
        JOIN
    lesson_4.friend_requests fr ON fr.initiator_user_id = f.friend_id
WHERE
    fr.target_user_id != 1 AND f.user_id = 1
        AND status = 'approved';